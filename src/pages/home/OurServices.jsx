            ...
            <div className="grid sm:grid-cols-2 grid-cols-1 gap-8 items-center">
                {
                    serviceLists.map((service) => (
                        <div key={service.id} className="shadow-md rounded-sm py-5 px-4 text-center space-y-2 text-green cursor-pointer hover:border hover:border-orange-400 transition-all duration-200">
                            <img src={service.img} alt="" className=" mx-auto text-green"/>
                            <h5 className="pt-3 font-semibold text-white"> {service.title}</h5>
                            <p className="text-zinc-400">{service.des}</p>
                        </div>
                    ))
                }
            </div>
