  ...
  const { refetch, data: orders = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://foodi-server-rcaa.onrender.com/payments/all`,
        {
          headers: {
            ...
            <tbody>
              {currentItems.map((item, index) => (
                <tr key={index}>
                  ...
                  {item.status  === "confirmed" ? <MdOutlineFileDownloadDone className="w-20 h-8 font-extrabold text-red" /> :  <button
                      className="btn bg-green text-white btn-xs text-center text-lg"
